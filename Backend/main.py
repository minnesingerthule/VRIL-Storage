...
# Разрешаем фронтенду (обычно на порту 5173 или 3000) стучаться к нам
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Для разработки можно так, в проде лучше указать конкретный домен
...
# --- Dependency: Получение текущего юзера ---
...
def register(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    # ВАЖНО: Создаем корневую папку для пользователя
...
def get_drive_state(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Конвертируем вручную (маппинг имен полей для фронтенда)
    folders_out = [
        schemas.FolderOut(
...
def upload_file(
    ...
    # Парсим folderId (может быть "null" строка или None)
